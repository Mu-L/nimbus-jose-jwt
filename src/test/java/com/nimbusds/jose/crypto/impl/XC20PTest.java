...
public class XC20PTest extends TestCase {
	...
	public void testDecryptException_modifiedCipherText() throws JOSEException {
		...
		byte[] modifiedCipherText = new byte[authenticatedCipherText.getCipherText().length];
		secureRandom.nextBytes(modifiedCipherText);
		
		try {
			XC20P.decryptAuthenticated(
				key,
				ivContainer.get(),
				modifiedCipherText,
				aad,
			...
			assertEquals("XChaCha20Poly1305 decryption failed: java.security.GeneralSecurityException: invalid MAC", e.getMessage());
			assertTrue(e.getCause() instanceof GeneralSecurityException);
			assertEquals("java.security.GeneralSecurityException: invalid MAC", e.getCause().getMessage());
		}
	}
	
	
	public void testDecryptException_modifiedAuthenticationTag() throws JOSEException {
		...
		assertEquals(XC20P.AUTH_TAG_BIT_LENGTH, ByteUtils.bitLength(authenticatedCipherText.getAuthenticationTag()));
		
		byte[] modifiedAuthenticationTag = new byte[XC20P.AUTH_TAG_BIT_LENGTH];
		secureRandom.nextBytes(modifiedAuthenticationTag);
	...
	public void testDecryptException_keyMismatch() throws JOSEException {
		...
		SecretKey otherKey = ContentCryptoProvider.generateCEK(EncryptionMethod.XC20P, secureRandom);
